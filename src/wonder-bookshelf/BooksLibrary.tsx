...
import { fetchLocalBooks, mergeLocalBooks, publicizeBooksForStore } from "./localBooks";
import {
...
export function BooksLibrary({
  ...
  const [books, setBooks] = useState<Book[]>(() => publicizeBooksForStore(loadBooks()));
  ...
  useEffect(() => {
    ...
    setBooks((current) => publicizeBooksForStore(mergeWonderBookPages(current, workspacePages)));
  }, [workspacePages]);
  ...
  const syncApple = useCallback(async (options?: { quiet?: boolean }) => {
    ...
    try {
      ...
      setBooks((current) => publicizeBooksForStore(mergeAppleBooks(current, result.books)));
      if (!options?.quiet) {
        setSync({
          state: "done",
  ...
  function readBook(id: string, _cfi?: string) {
    // Public site: never open EPUB/Ocean PDFs — store link only.
    const book = books.find((row) => row.id === id);
    if (book) openBookStore(book);
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Public site: never open EPUB reader — store / Amazon only.
      if (wantRead) {
        openBookStore(book);
      } else {
        setOpenId(book.id);
  ...
  // EPUB reader disabled on public site (Amazon / store only).
  if (open) {
